...
def quantifyer(log: pd.DataFrame, z: int, delta_t: timedelta, p_relative: list[float], p_absolute: list[int], projection: list[str] = ['A']):
    ...
    return risk_re_ident_quant(z=z, delta_t=delta_t, df_data=log, points_relative=p_relative, points_absoulute=p_absolute,projection=projection)
