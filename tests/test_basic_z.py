...
from zfilters.filtering_basic_z import apply_filter_wrapper as process
...
def evaluate(data, expected_outcome, z, delta_t):
    df = pd.DataFrame(data)
    expected = pd.DataFrame(expected_outcome).sort_values(by=[case_id, timestamp])
...
class TestZBasic(unittest.TestCase):
    ...
    def test_does_z_threshold_apply(self):
        sample_data = {
            case_id: [1, 2, 3, 4, 5],
            activity: ["A", "A", "A", "A", "A"],
        ...
        expected = {
            case_id: [3, 4, 5],
            activity: ["A", "A", "A"],
            timestamp: ["2024-11-1", "2024-11-1", "2024-11-1"],
            source: ["A", "A", "A"]
        }
